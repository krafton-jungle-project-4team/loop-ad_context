# Loop-Ad Team Common Contract v1.6 Final Reference
...
숙박 사이트마다 퍼널 이벤트가 다를 수 있으므로 퍼널은 하드코딩하지 않는다. 기본 호텔
퍼널 fixture를 제공하되 Dashboard에서 수정 가능해야 한다.
...
```text
프로모션별/세그먼트별 광고 콘텐츠를 생성하고, 생성 이유와 근거 리포트를 보여준다.
```
