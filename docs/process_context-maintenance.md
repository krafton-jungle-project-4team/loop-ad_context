...
이 평가는 `.agents/skills/loopad-ctx-contract-sync-checker/SKILL.md`를 기준으로 수행한다.
...
구조 검증은 `.agents/skills/loopad-ctx-structure-verifier/SKILL.md`를 기준으로 수행한다. 구조 검증은
내용 동기화 검사를 대체하지 않는다.
